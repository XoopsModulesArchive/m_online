...
{
    global $xoopsUser, $xoopsModule, $xoopsConfig, $HTTP_SERVER_VARS;

    include 'conta_tempo.php';
    ...
    if (false !== $onlines) {
        ...
        if (false !== $somembros) {
            ...
            for ($i = 0; $i < $total_membros; $i++) {
                ...
                $onlineUsers[$i]['tempo'] = conta_tempo($onlineUsers[$i]['user']->getVar('last_login'));
            }
            ...
            for ($i = 0; $i < $options[0]; $i++) {
                if (@is_object($onlineUsers[$i]['user'])) {
                    ...
                    if (1 == $options[4]) {
                        $bloco['content'] .= '<br><small>' . $onlineUsers[$i]['module'] . '<br>' . $onlineUsers[$i]['tempo'] . '</small>';
                    }

                    //Confere se está definido para mostrar o IP e se quem está vendo o bloco é administrador

                    if (1 == $administrador && 1 == $options[1]) {
                    ...
                    if (1 == $options[2]) {
                        ...
                        if ('' != $xoopsUser) {
                            ...
                            $bloco['content'] .= '<td class="odd" width="20%" align="right" valign="middle"><a href="javascript:openWithSelfMain(\''
                                                 . XOOPS_URL
                                                 . '/pmlite.php?send2=1&amp;to_userid='
                                                 . $onlineUsers[$i]['user']->getVar('uid')
                                                 . '\',\'pmlite\',450,370);"><img src="'
                                                 . XOOPS_URL
                                                 . '/images/icons/pm_small.gif" border="0" width="27" height="17" alt=""></a></td>';
...
{
    $membros = "<input type='text' name='options[]' value='" . (int)$options[0] . "'>";
